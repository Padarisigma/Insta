...
//     if (!isFollow) setOpen(true);

//     if (!open) {
//       try {
//         const response = await FollowByUserId(routID).unwrap();
// 		  setIsFollow(false)
//         refetch();
//       } catch (error) {
//         console.error(error);
...
//     }
  ...
  return (
    <div>
          ...
          {routId ? (
            <div className='flex gap-[20px] items-center'>
              <Button
                ...
                className={`px-[30px] py-[5px] ${isFollow ? 'bg-blue-500' : 'bg-[#313131]'} text-[#fff] rounded-md cursor-pointer`}
              >
      ...
      <UnfollowDialog open={open} setOpen={setOpen} username={userName} userId={routId} avatarUrl={''} />
    </div>
  );
};
